document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- DATABASE: SERVICE CONTENT (Do not remove this text) ---
    const serviceData = {
        "audio_music": {
        ...
        "audio_podcast": { // Matches your screenshot
    ...
    // --- DYNAMIC PAGE LOADER ---
    // This logic pulls the text above and injects it into service-detail.html
    if (window.location.pathname.includes('service-detail.html')) {
        ...
        if (id && serviceData[id]) {
            ...
            if (listContainer) {
                listContainer.innerHTML = ''; // Clear existing dummy content
                data.list.forEach((item, index) => {
                    const li = document.createElement('li');
                    // Adds a zero pad number (01, 02) for style
            ...
            // Fallback if ID is wrong (optional)
